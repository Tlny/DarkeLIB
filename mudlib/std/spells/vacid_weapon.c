...
if((int)at->query_property("vacid weapon") >= 25 ) {
    message("info", "This weapon can receive no more vacid weapon spells.", caster);
    caster->add_mp(props["mp cost"]);
    remove();
    return;
}
...
message("info", "You have finished enchanting!", caster);
message("info", "Uttering some magical incantations, "+(string)caster->query_cap_name()+" finishes enchanting a weapon with vacid.", 
environment(caster), ({ caster }));
