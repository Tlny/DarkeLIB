...
void finish_work(object from, object at) {
  ...
  wc["strike"] += skill / 6 + (random(skill)/5);
  wc_keys = keys(wc);
  i = sizeof(wc_keys);
  while(i--)
    at->set_wc(wc[wc_keys[i]], wc_keys[i]);
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
    message("info", "A %^BOLD%^%^ORANGE%^%^B_GREEN%^Striker%^RESET%^ rune appears on the weapon.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "striker" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
    if(pointerp(tmp)) tmp += ({ "A %^BOLD%^ORANGE%^%^B_GREEN%^Striker%^RESET%^ rune is inscribed on the weapon." });
    else if(stringp(tmp)) tmp = ({ tmp, "A %^BOLD%^ORANGE%^%^B_GREEN%^Striker%^RESET%^ rune is inscribed on the weapon." });
  else tmp = ({ "A %^BOLD%^ORANGE%^%^B_GREEN%^Striker%^RESET%^ rune is inscribed on the weapon." });
